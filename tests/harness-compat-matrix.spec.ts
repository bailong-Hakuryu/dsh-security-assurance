...
describe('declared Harness compatibility window', () => {
  it('is a closed supported set led by the primary qualification target', () => {
    ...
    expect(isSupportedHarnessVersion('0.1.3-alpha.2')).toBe(true)
    expect(isSupportedHarnessVersion('0.1.5-alpha.1')).toBe(true)
    expect(isSupportedHarnessVersion('0.1.5-alpha.2')).toBe(true)
    expect(isSupportedHarnessVersion('0.1.5-rc.1')).toBe(true)
    expect(isSupportedHarnessVersion('0.1.5-rc.2')).toBe(true)
  ...
  it('admits only a coherent runtime release and rejects supported-version skew', () => {
    ...
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({ status: 'SUPPORTED', version: '0.1.3-alpha.2' })

    expect(evaluateHarnessVersionAdmission([
      { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.5-alpha.1' },
      { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.5-alpha.1' },
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({ status: 'SUPPORTED', version: '0.1.5-alpha.1' })

    expect(evaluateHarnessVersionAdmission([
      { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.5-alpha.2' },
      { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.5-alpha.2' },
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({ status: 'SUPPORTED', version: '0.1.5-alpha.2' })

    expect(evaluateHarnessVersionAdmission([
      { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.5-rc.1' },
      { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.5-rc.1' },
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({ status: 'SUPPORTED', version: '0.1.5-rc.1' })

    expect(evaluateHarnessVersionAdmission([
      { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.5-rc.2' },
      { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.5-rc.2' },
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({ status: 'SUPPORTED', version: '0.1.5-rc.2' })

    expect(evaluateHarnessVersionAdmission([
      { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.6-alpha.1' },
      { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.6-alpha.1' },
    ], SUPPORTED_HARNESS_VERSIONS)).toEqual({
      status: 'UNSUPPORTED',
      packages: [
        { packageName: '@deepseek-ai/dsh-invariants', actual: '0.1.6-alpha.1' },
        { packageName: '@deepseek-ai/dsh-typert-registry', actual: '0.1.6-alpha.1' },
  ...
  it('matches the peer dependency ranges and the dual-plugin E2E script composition', () => {
    ...
    expect(packageJson.peerDependencies?.['@deepseek-ai/cordis-plugin-loader']).toBe('^1.0.2')
    for (const name of [
...
describe('harness-compat-matrix discovery', () => {
  it('builds the discovered matrix with the primary target on three operating systems', () => {
    ...
    expect(matrix.include).toHaveLength(28)
    ...
    for (const version of ['0.1.2-alpha.2', '0.1.2-alpha.3', '0.1.2-alpha.4', '0.1.2-alpha.5', '0.1.2-rc.1', '0.1.3-alpha.1', '0.1.3-alpha.2', '0.1.5-alpha.1', '0.1.5-alpha.2', '0.1.5-rc.1', '0.1.5-rc.2']) {
  ...
  it('admits a newly published Harness tag into verification automatically', () => {
    const result = runMatrix(['--tags-file', publishedTagsFixture(['0.1.6-alpha.1'])])

    expect(result.status).toBe(0)
    const matrix = parseMatrix(result.stdout)
    expect(matrix.include).toHaveLength(30)
    const recent = matrix.include.filter(lane => lane.track === 'recent')
    expect(recent).toHaveLength(2)
    expect(recent.every(lane => lane.harness === '0.1.6-alpha.1')).toBe(true)
  ...
  it('ranks a final release above prereleases when selecting recent versions', () => {
    const result = runMatrix(['--tags-file', publishedTagsFixture(['0.1.5'])])

    expect(result.status).toBe(0)
    const matrix = parseMatrix(result.stdout)
    const recent = matrix.include.filter(lane => lane.track === 'recent')
    expect(recent).toHaveLength(2)
    expect(recent.every(lane => lane.harness === '0.1.5')).toBe(true)
